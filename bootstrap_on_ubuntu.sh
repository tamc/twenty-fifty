...
sed -i '.bak' -e "s/'ruby'/'ruby1.9.1'/" setup.sh run.sh
sed -i '.bak' -e "s/'rake'/'rake1.9.1'/" setup.sh run.sh
sed -i '.bak' -e "s/\$rake gems:install/sudo \$rake gems:install/" setup.sh
./setup.sh
./run.sh
